...
class PrompTartTranslationsDataset(Dataset):
    def __init__(
        self,
        data_path: str,
        split: str,
    ...
    def get_dataset(self) -> Dict[str, List[Any]]:
        if self.split in ["train", "val", "test"]:
            if self.is_preprocessed:
                csv_path = f"{self.data_path}/{self.date}/preprocessed_dataset/{self.pretrained_model_name}/{self.split}.csv"
            else:
                csv_path = f"{self.data_path}/{self.date}/{self.split}.csv"
            data = pd.read_csv(csv_path)
            data = data.fillna("_")
        elif self.split == "predict":
            if self.is_preprocessed:
                csv_path = f"{self.data_path}/{self.date}/preprocessed_dataset/{self.pretrained_model_name}/test.csv"
            else:
                csv_path = f"{self.data_path}/{self.date}/test.csv"
            data = pd.read_csv(csv_path)
            data = data.fillna("_")
        ...
        instructions = data[self.instruction_column_name].tolist()
        if not self.is_causal:
